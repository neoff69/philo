...
/*   Updated: 2023/05/22 14:42:28 by vgonnot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_nbr_food_zero(int nbr_eat, pthread_mutex_t mutex_data)
{
...
int	check_if_finish(t_philo *philo, t_data *data)
{
	...
	while (i < philo[0].nbr_philo)
	{
		...
		eat_enough += is_nbr_food_zero(philo[i].nbr_eat, philo[i].mutex_data);
